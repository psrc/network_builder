...
if config['data_source'] == 'enterprise_gdb':
    ...
    print ('here')
    df_modeAttributes = gpd.read_file(config['file_gdb_path'], layer='modeAttributes')
    ...
    gdf_TransRefEdges= gpd.read_file(config['file_gdb_path'], layer='TransRefEdges', crs = crs)
    gdf_TransRefEdges = gdf_TransRefEdges.explode()
    gdf_TransRefEdges.index = gdf_TransRefEdges.index.droplevel(1)

    gdf_TransitLines = gpd.read_file(config['file_gdb_path'], layer='TransitLines', crs = crs)
    gdf_TransitLines = gdf_TransitLines.explode()
    gdf_TransitLines.index = gdf_TransitLines.index.droplevel(1)

    gdf_TransitPoints = gpd.read_file(config['file_gdb_path'], layer='TransitPoints', crs = crs)

    gdf_TurnMovements = gpd.read_file(config['file_gdb_path'], layer='TurnMovements', crs = crs)
    gdf_TurnMovements = gdf_TurnMovements.explode()
    gdf_TurnMovements.index = gdf_TurnMovements.index.droplevel(1)

    ## Juncions
    gdf_Junctions = gpd.read_file(config['file_gdb_path'], layer='TransRefJunctions', crs = crs)
    if config['build_transit_headways']:
        df_transit_frequencies = gpd.read_file(config['file_gdb_path'], layer='transitFrequency')
        df_transit_frequencies = df_transit_frequencies.drop('geometry', 1)


    if config['update_network_from_projects']:
        gdf_ProjectRoutes = gpd.read_file(config['file_gdb_path'], layer='ProjectRoutes', crs = crs)
        gdf_ProjectRoutes = gdf_ProjectRoutes.explode()
        gdf_ProjectRoutes.index = gdf_ProjectRoutes.index.droplevel(1)

        df_tblProjectsInScenarios = gpd.read_file(config['file_gdb_path'], layer='tblProjectsInScenarios')
        df_tblProjectsInScenarios = df_tblProjectsInScenarios.drop('geometry', 1)

        df_tblLineProjects = gpd.read_file(config['file_gdb_path'], layer='tblLineProjects')
        df_tblLineProjects = df_tblLineProjects.drop('geometry', 1)
        # point events (park and rides)
        df_evtPointProjectOutcomes =gpd.read_file(config['file_gdb_path'], layer='evtPointProjectOutcomes')
