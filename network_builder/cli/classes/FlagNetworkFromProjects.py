...
class FlagNetworkFromProjects(object):
    ...
    def _get_route_edges(self):
        ...
        edges = gpd.sjoin(
            self.network_gdf, buff_projects, how="inner", predicate="within"
        )

        edges = edges.sort_values(by=["InServiceDate_right"], ascending=False)

        edges["freq"] = edges.groupby("PSRCEdgeID")["PSRCEdgeID"].transform("count")
    ...
    def _edge_project_map(self):
        ...
        """
        return (
            self.route_edges.groupby(["PSRCEdgeID"])
            .apply(lambda x: list(x.projRteID))
            .to_dict()
        )
    ...
    def _get_route_junctions(self):
        ...
        node_list = list(set(node_list + self.route_edges.JNode.tolist()))
        return self.junctions_gdf[self.junctions_gdf.PSRCjunctID.isin(node_list)]
    ...
    def _junctions_to_coords(self):
        ...
        return {
            x.geometry.coords[0]: x.PSRCjunctID
            for x in self.route_junctions.itertuples()
        }
    ...
    def _edges_to_dict(self):
        ...
        for edge in self.route_edges.itertuples():
            ...
            if edge.Oneway_left == 0 or edge.Oneway_left == 1:
                edge_dict[(edge.INode, edge.JNode, edge.projRteID)] = {
                    "PSRCEdgeID": edge.PSRCEdgeID,
                    "dir": "IJ",
                }
            # Two way
                ...
                edge_dict[(edge.JNode, edge.INode, edge.projRteID)] = {
                    "PSRCEdgeID": edge.PSRCEdgeID,
                    "dir": "JI",
                }
            # Oneway JI
    ...
    def _flag_edges(self):
        ...
        for route_id in route_id_list:
            update_edges = self.route_edges[
                self.route_edges.projRteID == route_id
            ].PSRCEdgeID.tolist()
            ...
            if project.loc[project.index[0]].geometry.geom_type == "MultiLineString":
                ...
                project_coords = [
                    (x[0], x[1]) for x in project.loc[project.index[0]].geometry.coords
                ]

                # Add only coords that are spatialy coincident with junctions
                node_list = [
                    self.route_junctions_coords[x]
                    for x in project_coords
                    if x in self.route_junctions_coords.keys()
                ]
                            ...
                            if (
                                ...
                                if edge_id in self.edge_proj_dict.keys():
                                    self._logger.info(
                                        ...
                                        % (edge_id, self.edge_proj_dict[edge_id])
                                    )
                                else:
                                    self._logger.info(
                ...
                if edge_count == 0:
                    self._logger.info(
                        "Warning! No edges found as part of project %s." % (route_id)
                    )
                final_edge_count = initial_edge_count - len(update_edges)
                updated_edge_count_list.append(
                    {
                        "projRteID": route_id,
    ...
    def _update_edge_attributes(self):
        ...
        scenario_edges = self.network_gdf[
            ...
            | self.network_gdf.PSRCEdgeID.isin(project_edges.PSRCEdgeID.tolist())
        ]
        ...
        self._logger.info(
            "Removing %s edges deleted by projects"
            % (len(scenario_edges[scenario_edges["FacilityType"] == 99]))
        )
