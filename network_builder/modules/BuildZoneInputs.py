...
class BuildZoneInputs(object):
    ...
    def build_zone_inputs(self):
        zone_nodes_df = self.scenario_junctions.loc[
            self.scenario_junctions["is_zone"] == 1
        ]
        ...
        if self.config.update_network_from_projects:
            ...
            p_r_projects_df = self.point_events_df.loc[
                self.point_events_df["projRteID"].isin(p_r_projects)
            ].copy()
        ...
        zone_nodes_df = zone_nodes_df.rename(
            columns={"i": "Zone_id", "P_RStalls": "Capacity", "Processing": "External"}
        )
