...
scenario_edges = gdf_TransRefEdges.loc[((gdf_TransRefEdges[links['project_start_year']] <= config['model_year']) 
                                    & (gdf_TransRefEdges[links['active_link_flag']] > 0) 
                                    & (gdf_TransRefEdges[links['active_link_flag']] != 999))]
scenario_edges[links['project_id']] = 0
...
if config['update_network_from_projects']:
    ...
    scenario_edges = gdf_TransRefEdges.loc[((gdf_TransRefEdges[links['project_start_year']] <= config['model_year']) 
                                & (gdf_TransRefEdges[links['active_link_flag']] > 0) 
                                & (gdf_TransRefEdges[links['active_link_flag']] != 999))]
    scenario_edges[network_cols['project_id']] = 0
...
for turn in gdf_TurnMovements.iterrows():
    ...
    if from_edge.empty:
        ...
        continue
    elif int(from_edge[links['i_node']]) <> j_node:
        i_node = int(from_edge[links['i_node']])
    else:
        i_node = int(from_edge[links['j_node']])
    to_edge = scenario_edges[scenario_edges[links['id']] == turn[turn_cols['to_link_id']]]
    if to_edge.empty:
        logger.warning("Warning: To edge from Turn %s not found!" % (turn[turn_cols['id']]))
        continue
    elif int(to_edge[links['i_node']]) <> j_node:
        k_node = int(to_edge[links['i_node']])
    else:
        k_node = int(to_edge[links['j_node']])
