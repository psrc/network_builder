...
class ThinNetwork(object):
    ...
    def _compare_attributes(self, edge1, edge2, edge2_dir="IJ"):
        ...
        compare_atts1 = {
            key: value for (key, value) in edge1.items() if key in compare_cols
        }

        if edge2_dir == "IJ":
            compare_atts2 = {
                key: value for (key, value) in edge2.items() if key in compare_cols
            }
    ...
    def _report_duplicate_edges(self):
        ...
        reverse_net = reverse_net.rename(columns={"INode": "JNode", "JNode": "INode"})

        full_net = pd.concat([self.network_gdf, reverse_net])
        full_net["id"] = full_net.INode.astype(str) + "-" + full_net.JNode.astype(str)
    ...
    def _thin_network(self):
        ...
        merge_edges = merge_edges.rename(
            columns={"INode": "INode_y", "JNode": "JNode_y"}
        )
        one_way_keep = thin_edges.merge(
            merge_edges,
            how="inner",
            left_on=["INode", "JNode"],
        ...
        thin_edges = thin_edges[
            ~thin_edges["PSRCEdgeID"].isin(one_way_keep["PSRCEdgeID"].tolist())
        ]
                ...
                if merge:
                    ...
                    # Are lines digitized towards each other:
                        ...
                        if G.has_edge(merged_row["INode"], merged_row["JNode"]):
                            ...
                            G.remove_edge(edges[1][0], edges[1][1])
                            G.add_edge(
                                merged_row["INode"], merged_row["JNode"], **merged_row
                            )
    ...
    def _thin_junctions(self):
        ...
        keep_nodes = list(
            set(
                self.thinned_edges_gdf["INode"].tolist()
                + self.thinned_edges_gdf["JNode"].tolist()
            )
        )
        ...
        thinned_junctions["ScenarioNodeID"] = np.where(
            thinned_junctions["EMME2nodeID"] > 0,
            thinned_junctions["EMME2nodeID"],
            thinned_junctions["ScenarioNodeID"],
        )
        # Now make a map of old to new
        recode_dict = pd.Series(
            thinned_junctions.ScenarioNodeID.values,
            thinned_junctions.PSRCjunctID.values,
        ).to_dict()
        # Recode_edges
    ...
    def _check_edge_connection_validity(self, node, edges, network_graph):
        ...
        if len(edges) != 2:
            self._logger.warning(
                "WARNING: Node " + str(node) + " does not have exactly two edges!"
            )
            return None

        else:
            edge_1 = network_graph.get_edge_data(edges[0][0], edges[0][1]).copy()

            edge_2 = network_graph.get_edge_data(edges[1][0], edges[1][1]).copy()
