...
class BuildHOVSystem(object):
    ...
    def _get_hov_junctions(self):
        ...
        keep_nodes = list(
            set(self.hov_edges["INode"].tolist() + self.hov_edges["JNode"].tolist())
        )

        hov_junctions = self.junctions_gdf[
        ...
        hov_junctions["ScenarioNodeID"] = range(
            ...
            self.junctions_gdf.ScenarioNodeID.max().astype(int)
            + len(hov_junctions)
            + 1,
        )
    ...
    def _update_hov_ij_nodes(self, hov_junctions):
        ...
        """
        recode_dict = dict(
            zip(hov_junctions["PSRCjunctID"], hov_junctions["ScenarioNodeID"])
        )
    ...
    def _get_weave_edges(self):
        ...
        weave_edges = self.hov_junctions[["PSRCjunctID", "geometry", "ScenarioNodeID"]]
